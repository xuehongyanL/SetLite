let DataFrame = function(cols){
  ...
  this.add = function(record){
    for(let col of this.columns) this.values[col].push(record[col]);
    this.length++;
  };
  this.drop = function(idx){
    for(let col of this.columns) this.values[col].splice(idx,1);
    this.length--;
  };
  ...
  this.to_array = function(){
    ...
    return Array(this.length).fill(null).map(
      function(value,index){
        let dat = [];
        for(let col of self.columns) dat.push(self.values[col][index]);
        return dat;
      }
    );
  };
  ...
  this.to_csv = function(){
    return this.to_array().map((value,index)=> {return value.join();}).join('\r\n');
  };
};
